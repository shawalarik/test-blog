import { defineConfig } from "eslint/config"; // 规范化 ESLint 配置的写法，提供类型提示和自动补全
import eslint from "@eslint/js"; //  ESLint 官方的核心规则集（@eslint/js），是 ESLint 功能的基础
import globals from "globals"; // 提供不同运行环境（浏览器、Node.js、ES6 等）的预定义全局变量列表
//import projectGlobals from "./eslintrc-globals.js";
import pluginVue from "eslint-plugin-vue"; // Vue 官方的 ESLint 插件，专门用于检查 Vue 单文件组件（SFC）的语法和规范
import * as parserVue from "vue-eslint-parser"; // 导入 Vue 专用的解析器，用于解析 .vue 文件的语法结构
import configPrettier from "eslint-config-prettier"; // 用于关闭 ESLint 中与 Prettier 格式化规则冲突的部分，确保 ESLint 和 Prettier 协同工作时不会互相干扰
import pluginPrettier from "eslint-plugin-prettier"; // 将 Prettier 的格式化规则集成到 ESLint 的检查流程中，让 ESLint 不仅能检查代码质量，还能同时检查代码格式是否符合 Prettier 规范
import pluginTypeScript from "@typescript-eslint/eslint-plugin"; // TypeScript 专用的 ESLint 插件，提供 TypeScript 语法相关的检查规则
import * as parserTypeScript from "@typescript-eslint/parser"; // 导入 TypeScript 专用的解析器，用于解析 TypeScript 代码
