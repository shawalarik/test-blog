...
class FileRenamer {
  private options: FileRenamerOptions;
  constructor(options = {}) {
    this.options = {
      directory: ".", // 默认处理当前目录
  ...
  async execute() {
    try {
      ...
      // 排序文件（如果需要）
      const sortedFiles = this.options.sortFiles ? await this._sortFilesByModifiedTime(targetFiles) : targetFiles;
  ...
  async _sortFilesByModifiedTime(files) {
    const fileStats = await Promise.all(
      files.map(file =>
        fs
          .stat(path.join(this.options.directory, file))
