...
::: note 本内容基于官网的安装步骤，目前是最新的安装步骤(2025-05-17)。最新安装步骤请前往 [Docker 安装文档](https://docs.docker.com/engine/install/centos/)
...
必须启用该 centos-extras 存储库。此存储库默认启用。如果您已禁用它，则需要重新启用它。
...
如果 `dnf` 报告没有安装这些软件包，那也没关系，只是说明曾经没安装过，不影响任何东西。
...
::: tip 提示
Docker 安装路径是 `/var/lib/docker`，安装 Docker 成功后，在该路径中有一个 image 和 containers 文件夹，分别存放镜像和实例。
:::

您可以根据需要以不同方式安装 Docker Engine：

- 从 [Docker 的存储库](https://docs.docker.com/engine/install/centos/#install-using-the-repository)或者其他镜像库里下载，这是最推荐的方法

- 下载 RPM 包并[手动安装](https://docs.docker.com/engine/install/centos/#install-from-a-package)并完全手动管理升级。这在某些情况下非常有用，例如在无法访问互联网的气隙系统上安装 Docker

- [便捷脚本](https://docs.docker.com/engine/install/centos/#install-using-the-convenience-script)安装 Docker，可以在测试和开发环境中安装
...
![1747484505927.png](https://img.seasir.top/2025/05/17/20/21/45/1747484505927_1747484505958.png)
...
![1747484720676.png](https://img.seasir.top/2025/05/17/20/25/20/1747484720676_1747484720773.png)
